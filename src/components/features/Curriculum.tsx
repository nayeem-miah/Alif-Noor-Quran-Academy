...
import { learningCards } from "./curriculumData";
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 w-full">
        ...
        <div className="text-center max-w-2xl mx-auto mb-12 sm:mb-16 animate-fade-in">
          <h2 className="font-serif text-3xl sm:text-4xl font-bold tracking-tight text-secondary-dark">
            {t("আমি যা যা শেখাই", "What I Teach")}
          </h2>
          ...
          <p className="text-text-secondary text-sm sm:text-base font-semibold mt-4 leading-relaxed">
            {t(
              ...
              "A structured curriculum designed to strengthen the basic foundation of your child's Islamic education."
            )}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 mb-12 animate-fade-in-up">
          {learningCards.map((card, idx) => {
            const cardTitle = t(card.titleBn, card.titleEn);
            const cardSubtitle = card.subtitleBn && card.subtitleEn ? t(card.subtitleBn, card.subtitleEn) : "";
            const cardDesc = t(card.descBn, card.descEn);
            const cardItems = card.itemsBn && card.itemsEn ? (language === "bn" ? card.itemsBn : card.itemsEn) : null;
