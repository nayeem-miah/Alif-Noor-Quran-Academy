  ...
  return (
    <section id="pricing" className="w-full py-16 sm:py-24 bg-background-warm flex items-center justify-center border-t border-border-subtle relative overflow-x-hidden">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 w-full relative z-10">
        ...
        {/* Split Layout: Pricing Grid (left) & Trial Illustration Card (right) */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-12 animate-fade-in-up items-stretch">
          {/* Pricing Cards Grid (Left 9 cols) */}
          <div className="lg:col-span-9 grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8 items-stretch">
            {plans.map((plan) => {
          ...
          {/* Trial/Promo Card (Right 3 cols) */}
          <div className="lg:col-span-3 flex">
            <div className="bg-[#FAF8F2] border border-border-subtle rounded-3xl p-6 flex flex-col justify-between items-center text-center shadow-premium-sm hover:shadow-premium hover:border-primary/20 transition-all duration-300 w-full relative overflow-hidden group hover:-translate-y-2">
              ...
              <div>
                <span className="bg-emerald-100 text-emerald-800 text-[10px] font-extrabold uppercase px-3 py-1 rounded-full tracking-wider shadow-sm border border-emerald-200">
                  {t("ফ্রি ট্রায়াল", "Free Trial")}
                </span>
                
                <h3 className="font-serif text-lg font-bold text-primary mt-4">
                  {t("১ম ক্লাস ফ্রি!", "1st Class Free!")}
                </h3>
                <p className="text-[10px] font-bold text-text-secondary/55 uppercase tracking-wider mt-1">
                  {t("কোনো ফি ছাড়াই যাচাই করুন", "Try without commitment")}
                </p>

                {/* Illustration Container */}
                <div className="relative w-full aspect-square max-w-[150px] mx-auto my-5 transform transition-transform duration-500 group-hover:scale-105 animate-float-slow">
                  <Image
                    src="/images/pricing_trial.png"
                    alt={t("ফ্রি ট্রায়াল ক্লাস", "Free Trial Class")}
                    fill
                ...
                <p className="text-text-secondary text-xs leading-relaxed font-semibold">
                  {t(
                    "আপনার সন্তানের জন্য অনলাইন ক্লাসটি কেমন হবে তা যাচাই করতে আজই একটি ফ্রি ট্রায়াল ক্লাস বুক করুন।",
                    "Book a free trial class today to see how the online lessons fit your child's needs."
                  )}
                </p>
              </div>

              <Link
                href={`https://wa.me/8801852077834?text=${encodeURIComponent(
                  t(
                    "আসসালামু আলাইকুম, আমি আমার সন্তানের জন্য একটি ফ্রি ট্রায়াল ক্লাস বুক করতে আগ্রহী।",
                    "Assalamu Alaikum, I am interested in booking a Free Trial Class for my child."
                  )
                )}`}
                target="_blank"
                className="w-full mt-6 py-3 px-4 rounded-xl font-bold text-center text-xs bg-primary text-background-warm hover:bg-primary-hover shadow-premium-sm transition-all duration-200 cursor-pointer flex items-center justify-center gap-1.5 active:scale-98"
              >
                <span>{t("ফ্রি ক্লাস বুক করুন", "Book Free Trial")}</span>
              </Link>
            </div>
          </div>
        </div>
